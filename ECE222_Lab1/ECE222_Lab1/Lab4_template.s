...
__MAIN
			...
			; ORR		R1, R1, #0x400
			; STR		R1, [R10]
			LDR		R10, =IO2IntEnf			; Address to enable interrupt on P2.10 for falling edge
			LDR		R1, [R10]
			ORR		R1, R1, #0x400			; Setting bit 10
			STR		R1, [R10]				; Now the interrupt on P2.10 for falling edge is enabl
		...
		; They just turn off all LEDs 
			...
			MOV		R2, #0x9C4				; Putting 2500 into R2 to do the right 0.1mS delay (0.25s)
			MOV		R3, #0x00000004			; Turn on 4 left LEDs (Port 2)
			STR		R3, [R10, #0x40]		; Turn on 4 left LEDs (Port 2)
		...
		; Initializes R11 to a 16-bit non-zero value and NOTHING else can write to R11 !!
			...
			BL 		RNG  
			MOV		R9, #2500
			UDIV	R11, R11, R9			; The amount of time/10000 = amount of seconds. the amount of time*4= how many times call the dealy of 0.25s *4/10000 = /2500
LOOP1		BL		DELAY
		;FAZER UM LOAD EM R3
			...
			STR		R3, [R10, #0x40]
			SUBS	R11, R11, #1			; Decrementing R11
			BNE		LOOP1					; Checking R11
			MOV 	R3, #0xB0000000			; Turn off three LEDs on port 1  
			STR 	R3, [r10, #0x20]
			MOV 	R3, #0x0000007C
			STR 	R3, [R10, #0x40] 		; Turn off five LEDs on port 2
			MOV		R4, #0x00				; R4 will be used as the time counter
			MOV		R5, #0x00				; R5 will be used to monitor when turning on/off the LEDs.
			MOV		R2, #1					; To do delay of 0.1ms
LOOP2		BL		DELAY
			CMP		R6, #0
			BEQ		STOP_COUNT
			ADD		R4, R4, #1				; Increment the time counter
			ADD		R5, R5, #1				; Counter to monitor when turn on/off the LEDS
			TEQ		R5, #1000				; To check intervals of time of 1s
			BNE		LOOP2					; If it didn't complete 1000*0.1ms=0.1s, go to loop2, otherwise, turn on/off the LEDs
			MOV		R5, #0x00
			LDR		R3, [R10, #0x20]		; Getting the value of port 1
			EOR		R3, #0xB0000000
			STR		R3, [R10, #0x20]
